...
│  9. [선택] Obsidian CLI 연동                     │
│     → Daily Note append + 미완료 태스크 보고     │
│ 10. Git 커밋 (자동 생성 메시지 + 사용자 확인)    │
│ 11. 출력: 최종 보고서 + 다음 세션 작업           │
└─────────────────────────────────────────────────┘
```
...
**Step 0 — 프로젝트 맵 로딩 (v1.1.0+):**

`work-tree.md`를 탐색하여 프로젝트 간 이동 맵을 구축합니다:
1. 현재 디렉토리 → 상위 디렉토리 (최대 3단계) → `~/work-tree.md`
2. 발견 시 하위 프로젝트 경로와 work_logs 상태를 로딩하여 세션 중 프로젝트 전환 지원

**로딩 순서:**
1. `work-tree.md` — 프로젝트 맵 (프로젝트 간 네비게이션)
2. `remind.md` — 프로젝트 규칙
3. `error-rules.md` — 에러 방지 규칙 (프로젝트 레벨)
4. `chatlog.md` — 이전 세션 + 미완료 작업
5. `CHANGELOG.md` — 최근 변경사항
6. `~/work_logs/error-rules.md` — 글로벌 공유 규칙 (있는 경우)
...
| 6.5 | Obsidian CLI (선택, v4) | Daily Note + 태스크 |
| 7 | Git 커밋 (사용자 확인 후) | — |
| 8 | 최종 보고서 출력 | — |

**Obsidian CLI 연동 (v4, 선택):**

[Obsidian CLI](https://github.com/anthropics/obsidian-cli)가 설치되어 있고 앱이 실행 중인 경우:
- Daily Note에 세션 요약 append
- Vault 전체 미완료 태스크 수 보고
- 작업일지 Property 설정 (status, session, project)
- CLI 없으면 자동 스킵 — 파일 기반 기록이 항상 우선
